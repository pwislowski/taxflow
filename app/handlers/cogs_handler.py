...
class CogsHandler(BaseHandler):
    def __init__(
        self,
        df: pd.DataFrame,
        column: Optional[OtpSegmentedPnlColumns] = OtpSegmentedPnlColumns.Cogs,
        ...
        if base_total == 0:
            if target_total == 0:
                self.logger.debug(
                    "allocation_skipped",
                    handler=type(self).__name__,
                    reason="base_and_target_zero",
                )
                return
            self.logger.debug(
                "allocation_skipped",
                handler=type(self).__name__,
                reason="base_zero_target_nonzero",
            )
            return
    ...
    def process(self) -> pd.DataFrame:
        """Apply COGS allocation to the DataFrame.

        Identifies relevant rows based on P&L item types (e.g., COGS),
        ...
        Returns:
            pd.DataFrame: The segmented DataFrame with allocated COGS.

        Raises:
            KeyError: If required columns like Category or COGS are missing in CogsHandler.
            ValueError: If allocation rules lead to invalid distributions (e.g., division by zero) in CogsHandler.
        """
        self.logger.info(
